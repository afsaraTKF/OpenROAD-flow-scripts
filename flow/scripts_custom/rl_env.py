...
class RoutingEnv(gym.Env):
    ...
    def __init__(self, csv_data, box_violations):
        ...
        # For observations we combine chip-level features and simple box summary stats in a single vector
        # Features: [iteration, pin_count, net_count, total_drv, wireLength, box_mean, box_var]
        obs_low = np.array([0, 0, 0, 0, 0, 0, 0], dtype=np.float32)
        obs_high = np.array([np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf], dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        obs = np.concatenate([chip_features, box_stats])
    ...
    def step(self, action):
        ...
        # Define a better reward function that considers:
        # 1. Violation reduction from previous iteration
        # 2. Current iteration number (earlier convergence is better)
        # 3. Spatial consistency
        # 4. Total remaining violations
        
        # Get previous total violations
        prev_total = np.sum(current_violations)
        
        # Calculate violation reduction (relative to previous)
        violation_reduction = prev_total - total_violation if prev_total > 0 else 0
        reduction_rate = violation_reduction / prev_total if prev_total > 0 else 0
        
        # Calculate iteration penalty (exponentially increasing with iteration)
        iteration_penalty = 0.1 * np.exp(self.current_iteration / 10)
        
        # Calculate spatial consistency penalty
        spatial_penalty = np.var(neighbor_ratios) * 0.1
        
        # Base reward is the reduction rate
        reward = reduction_rate * 1000  # Scale up the reward for better learning
        
        # Penalize based on iteration count and spatial consistency
        reward -= iteration_penalty
        reward -= spatial_penalty
        
        # Add bonus for reaching zero violations
        if total_violation == 0:
            reward += 1000 / (self.current_iteration + 1)  # Bigger bonus for earlier convergence
        
        # Add penalty for remaining violations (normalized by initial violations)
        initial_violations = np.sum(self.violation_history[0])
        if initial_violations > 0:
            remaining_penalty = (total_violation / initial_violations) * 100
        ...
        print(f"   Reduction rate: {reduction_rate:.4f}, Iteration penalty: {iteration_penalty:.4f}")
